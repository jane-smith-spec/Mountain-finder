...
| **P7.4 Integration** *(decision layer done 2026-08-17; app wiring open)* | `suggestPoseTrim` in `src/pipeline/cv-alignment.ts`: near-field-free profile REQUIRED (checked from the profile's own distances), heading search clamped to a compass budget (default ±6° — a wider search was measured returning a confident impostor 10.4° off, CV-10), result is a suggestion for the **visible** P5.1 sliders, never a hidden correction. App wiring remains | `npx vitest run src/pipeline/cv-alignment.test.ts` (13, incl. the impostor-policy refusal); on the real frame: heading err −0.208° (beats the compass's 0.596°), pitch err +0.817° (beats assumed zero's 3.52°) — label error strictly improves; the original acceptance bar applies to the app wiring when it lands |
