...
2. **CV upgrade (v2.1) — 🔨 in progress.** The computed silhouette auto-aligns to the photo's actual skyline; label error drops accordingly, measured against the ground-truth set. The aligner meets its bar on computed profiles (0.013°). On the one real photograph with a known pose it recovers **heading to 0.207°** — closer than the phone's own magnetometer — and **pitch to only 1.97°**, the extractor's remaining bias landing almost entirely there. See [docs/FINDINGS.md](docs/FINDINGS.md) CV-2…CV-7 and [docs/REAL-PHOTO-POSE.md](docs/REAL-PHOTO-POSE.md).
