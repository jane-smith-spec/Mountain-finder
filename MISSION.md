...
3. **Live view (v3) — 🔨 pure layers proven, shell not built.** A mobile app (Expo, reusing the core) does the same continuously against the camera feed with sensor fusion. Both computational halves exist and are tested — `src/live/sensors.ts` (pose from gravity and compass, with the EXIF path's refusals) and `src/live/loop.ts` (one scene re-projected per tick, proven identical to re-running the still pipeline, refusing when the camera turns past swept terrain). The shell itself is deliberately unbuilt in an environment whose toolchain cannot run its self-check.
