...
| **P7.7** | Pitch is unrecorded and always wrong at zero | EXIF carries no pitch, `--pitch` defaults to 0, and in all three annotated photographs the drawn horizon sits low because of it — measured at Railroad Ridge as **−3.52°**, the largest single error in that render. Options, none chosen: recover it from the aligner (its pitch is the half that is currently bad, [CV-6](docs/FINDINGS.md)); expose it as a trim slider the user nudges (P5.1 already has the machinery); or estimate it from the fraction of the frame below the computed horizon. The first is the right answer if P7.5 lands |
